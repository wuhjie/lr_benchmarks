...
try:
    from .backend_pool import BackendPool, clone_backends, parse_backends
...
DEFAULT_SCORER_URLS = (
    ...
    "http://localhost:9001/classify",
    "http://localhost:9002/classify",
    "http://localhost:9003/classify",
)
...
class PaperSearchV2Agent:
    def __init__(self, logger: Any = None, *args, **kwargs):
        ...
        self.agent_backends = clone_backends(agent_backends or _parse_agent_backends())
        self.agent_pool = BackendPool(self.agent_backends, backend_kind="agent")
        ...
        if warmup_agent_backends is not None:
            self.warmup_agent_backends = clone_backends(warmup_agent_backends)
        elif (
            ...
            self.warmup_agent_backends = parse_backends(
            ...
            self.warmup_agent_backends = clone_backends(self.agent_backends)
        self.warmup_agent_pool = BackendPool(self.warmup_agent_backends, backend_kind="warmup_agent")

        scorer_backends = kwargs.get("scorer_backends")
        self.scorer_backends = clone_backends(scorer_backends or _parse_scorer_backends())
        self.scorer_pool = BackendPool(self.scorer_backends, backend_kind="scorer")

        self.selector_url = self.scorer_pool.first_url()
        self.selector_model_name = kwargs.get(
            "selector_model_name", os.getenv("PAPER_AGENT_V2_SELECTOR_MODEL", "selector_qwen3_8b")
        )
    ...
    def _reserve_agent(self, excluded: set[str] | None = None) -> dict[str, Any]:
        return self.agent_pool.reserve(excluded=excluded)

    def _reserve_warmup_agent(self, excluded: set[str] | None = None) -> dict[str, Any]:
        return self.warmup_agent_pool.reserve(excluded=excluded)

    def _reserve_scorer(self, excluded: set[str] | None = None) -> dict[str, Any]:
        return self.scorer_pool.reserve(excluded=excluded)
